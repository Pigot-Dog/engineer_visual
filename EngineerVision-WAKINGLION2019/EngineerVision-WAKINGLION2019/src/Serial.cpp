...
int Serial::openDevice(const char *dev_name,
             int baud_rate,
             int databits,
             int stopbits,
             int parity)
{
    ...
    setDevice(baud_rate,databits,stopbits,parity);

    return fd;
}

int Serial::openDevice(DeviceParam port_param)
{
    this->port_param=port_param;

    fd = open(this->port_param.dev_name, O_RDWR | O_NONBLOCK);//| O_NOCTTY | O_NDELAY
    if (-1 == fd)
    {
        perror("Can't Open Serial Port");
        return -1;
...
{
    fd = open(port_param.dev_name, O_RDWR | O_NONBLOCK);//| O_NOCTTY | O_NDELAY
    if (-1 == fd)
    {
        perror("Can't Open Serial Port");
        return -1;
...
bool Serial::closeDevice(int fd)
{
    close(fd);
}
...
bool Serial::setDevice(int baud_rate,int databits,int stopbits,int parity)
{
    if(setBaudRate(baud_rate))
        if(setParity(databits,stopbits,parity))
            return true;
    return false;
}
